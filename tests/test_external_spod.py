...
def _field_snapshots(regen, fixture_doc: dict) -> np.ndarray:
    """Manufactured snapshots, after asserting the vendored field fingerprint."""
    snapshots = regen.manufactured_field()
    got = regen.snapshot_sha256(snapshots)
    want = fixture_doc["construction"]["field_sha256"]
    assert got == want, (
        f"snapshot field sha256 {got} != fixture {want}; "
...
def test_rebuilt_field_matches_fixture_fingerprint(regen, fixture_doc) -> None:
    _field_snapshots(regen, fixture_doc)
...
def test_openmodalpy_matches_vendored_pyspod(regen, fixture_doc, tmp_path: Path, bin_idx: int, mode_idx: int) -> None:
    q = _field_snapshots(regen, fixture_doc)
...
@pytest.mark.parametrize(("bin_idx", "mode_idx"), OCCUPIED)
def test_openmodalpy_matches_closed_form(regen, fixture_doc, tmp_path: Path, bin_idx: int, mode_idx: int) -> None:
    q = _field_snapshots(regen, fixture_doc)
    got = float(_openmodalpy_eigs(q, fixture_doc, tmp_path)[bin_idx, mode_idx])
...
def test_fixture_provenance_records_both_solvers_and_the_pinned_pyspod(fixture_doc) -> None:
    ...
    assert construction["field_sha256"]
