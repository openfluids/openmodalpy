...
def float64_bits_equal(left: np.ndarray, right: np.ndarray) -> bool:
    """True iff both arrays are the same shape and the same float64 bit pattern."""
    a = np.ascontiguousarray(left, dtype=np.float64)
    b = np.ascontiguousarray(right, dtype=np.float64)
    return a.shape == b.shape and a.tobytes() == b.tobytes()


def snapshots_to_json(snapshots: np.ndarray) -> list:
    """Encode a float64 field as nested JSON numbers (lossless via Python float repr)."""
    return np.ascontiguousarray(snapshots, dtype=np.float64).tolist()


def snapshots_from_json(payload) -> np.ndarray:
    """Decode a vendored snapshot field to float64."""
    return np.asarray(payload, dtype=np.float64)


def assert_written_field_is_passed_field(payload, passed: np.ndarray, *, label: str) -> None:
    """Refuse to write if the fixture field is not the array given to PyDMD."""
    written = snapshots_from_json(payload)
    if not float64_bits_equal(written, passed):
        raise SystemExit(f"REFUSING TO WRITE: {label} fixture field is not the array passed to the reference package")
    round_tripped = snapshots_from_json(json.loads(json.dumps(payload)))
    if not float64_bits_equal(round_tripped, passed):
...
def _build_document(dmd_cls, pydmd_version: str) -> dict:
    ...
    cases: dict[str, dict] = {}
    passed_fields: dict[str, np.ndarray] = {}
    for case_name, noise in (("noiseless", 0.0), ("noise_1e-3", NOISE_RELATIVE_RMS)):
        ...
        passed_fields[case_name] = snapshots
    ...
    for case_name, original in passed_fields.items():
        assert_written_field_is_passed_field(
            document["cases"][case_name]["snapshots"],
            original,
            label=case_name,
        )
    return document
